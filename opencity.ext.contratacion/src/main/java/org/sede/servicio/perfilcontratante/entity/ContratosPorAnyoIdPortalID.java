package org.sede.servicio.perfilcontratante.entity;


import javax.persistence.Column;
import javax.persistence.Embeddable;
import javax.validation.constraints.Digits;
import java.io.Serializable;
import java.math.BigDecimal;

@Embeddable
public class ContratosPorAnyoIdPortalID implements Serializable {
    @Column(name = "ID_PORTAL", nullable = false)
    @Digits(integer = 6, fraction = 0)
    private BigDecimal idPortal;
    @Column(name = "ANYO", nullable = true)
    private String anyo;
    public BigDecimal getIdPortal() {
        return idPortal;
    }
    public void setIdPortal(BigDecimal idPortal) {
        this.idPortal = idPortal;
    }
    public String getAnyo() {
        return anyo;
    }
    public void setAnyo(String anyo) {
        this.anyo = anyo;
    }
    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (!(o instanceof ContratosPorAnyoIdPortalID)) return false;

        ContratosPorAnyoIdPortalID that = (ContratosPorAnyoIdPortalID) o;

        if (!getIdPortal().equals(that.getIdPortal())) return false;
        return getAnyo().equals(that.getAnyo());

    }
    @Override
    public int hashCode() {
        int result = getIdPortal().hashCode();
        result = 31 * result + getAnyo().hashCode();
        return result;
    }
}