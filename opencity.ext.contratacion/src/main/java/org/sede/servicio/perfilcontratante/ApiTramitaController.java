...
public class ApiTramitaController {
    ...
    private static final String SERVICIO = "contratacion-publica/api-tramita";
    ...
    @RequestMapping(path = "/{id}", method = RequestMethod.GET, produces = {
            MediaType.TEXT_HTML_VALUE, "*/*"})
    public String eliminar(Model model,@PathVariable BigDecimal id) throws SearchParseException {
        model.addAttribute(ModelAttr.RESULTADO, ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Metodo no implementado"));
    ...
    public String crear(Model model,@RequestBody String tramita) throws Exception {
        Contrato entity = new Contrato(tramita,Funciones.getPeticion());
        model.addAttribute(ModelAttr.RESULTADO, apiCrear(entity));
        return MAPPING + "/index";
    }*/
                    ...
                    if (actualObj.has("pc:tender")) {
                        ...
                        if (jsonNode.isArray()) {
                            ...
                            while (iterator.hasNext()) {
                                JsonNode valor = iterator.next();
                                if (valor.has("pproc:awardDate")) {
                                    Oferta ofertaGanadora = rellenarOferta(valor, peticion, true, "");
                                    con.getOfertas().add(ofertaGanadora);
                                } else {
                                    Oferta ofertaGanadora = rellenarOferta(valor, peticion, false, "");
                                    con.getOfertas().add(ofertaGanadora);
                                }
                            }
                        }
                            ...
                            if (loteAdjudicacion.has("pc:tender")) {
                                ...
                                if (jsonNode.isArray()) {
                                    ...
                                    while (iterator.hasNext()) {
                                        JsonNode valor = iterator.next();
                                        if (valor.has("pproc:awardDate")) {
                                            Oferta ofertaGanadora = rellenarOferta(valor, peticion, true, idLote);
                                            con.getOfertas().add(ofertaGanadora);
                                            con.setFechaContrato(ofertaGanadora.getFechaAdjudicacion());
                                        } else {
                                            Oferta ofertaGanadora = rellenarOferta(valor, peticion, false, idLote);
                                            con.getOfertas().add(ofertaGanadora);
                                        }
                                    }
                                }
                    ...
                    if (actualObj.get("pc:supplier").has("org:identifier")) {
                        ...
                        if (licitador != null) {
                            ...
                            if (actualObj.get("pc:supplier").has("org:identifier")) {
                                ...
                                if (Oferta.anoniCierto(licitador.getNif())) {
